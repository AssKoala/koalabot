...
import profanities from './../data/profanity.json' assert { type: 'json' }
...
{
    const start = Common.startTiming("recalculateProfanityLeaders(): ");
    ...
    try {
        ...
        Common.logError(`Failed to calculate profanity leaders, got ${e}`);
    }
...
function addMessageToProfanityLeaderboard(discordStenographerMsg)
{
    ...
    try {
        ...
        Common.logError(`Failed to add message to profanity leaderboard, got ${e}`);
        result = [];
    }
...
function getProfanityLeader(profanity, perCapita = false, ignoreList = [])
{
    try {
        ...
        Common.logError(`Failed to get profanity leader for ${profanity}, got error ${e}`);
    }
...
async function showProfanityLeaderboard(interaction, perCapita = false)
{
    try {
        ...
        Common.logError(`Exception getting profanity leaderboard, got ${e}`, interaction, true);
    }
}

function updateProfanityLeaderboard(message)
{
    const start = Common.startTiming("updateProfanityLeaderboard(): ");

    try {
        if (message.author.bot) return;

        const stdMsg = Common.getStandardDiscordMessageFormat(message);
        ...
        Common.logError(`Failed to update profanity leaderboard, got ${e}`);
    }
...
async function handleDisplayLeaderboardCommand(interaction, options)
{
    try {
        ...
        await Common.logError(`Failed to handle leaderboard display command, got ${e}`, interaction, true);
    }
...
async function handleDisplayCustomLeaderboardCommand(interaction, options, ignoreList = [])
{
    try {
        ...
        options.forEach((opt) => {
            switch (opt.name) {
                ...
                case `per_capita`:
                    ...
                    break;
                default:
                    Common.logError(`Unexpected value when displaying custom leaderboard: ${opt.name}`);
                    break;
            }
        });
        ...
        await Common.logError(`Failed to handle custom leaderboard, got ${e}`, interaction, true);
    }
}

async function handleLeaderboardCommand(interaction)
{
    const start = Common.startTiming("handleLeaderboardCommand(): ");
...
function registerLeaderboardCommand(client) {
    ...
    Common.registerMessageListener(updateProfanityLeaderboard);
}

Common.registerCommandModule(registerLeaderboardCommand, getLeaderboardJSON);
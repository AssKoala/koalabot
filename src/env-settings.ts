...
export function registerEnvironmentSettings(settingsManager: SettingsManager): void {
    // global settings
    settingsManager.register('global', "BOT_NAME", "BottyMcBotFace", "Name for the bot to use when referencing self", false);
    settingsManager.register('global', "DEBUG_ENABLE", "false", "Set to true to enable debug functionality", false);
    settingsManager.register('global', "COMMAND_LIST", "settings,coinflip,diceroll,leaderboard", "Comma separate list of commands to load.  All commands are expected to be in the ./commands folder and implement the DiscordBotCommand interface.  Commands are dynamically imported so long as they register their name in .env and the command file itself has a registerDiscordBotCommand call to assign an instance to a given command.", false);
    ...
    settingsManager.register('global', 'LISTENER_LIST', '', 'List of listener modules to load', false);
    ...
    settingsManager.register('discord', "DISCORD_GUILD_ID", "", "Comma separate list of guilds the bot will join.  e.g.\n\tDISCORD_GUILD_ID=\"12345\" is a single server.\nDISCORD_GUILD_ID=\"12345,67891\" for two servers and so on.", true);
